...
export function KnowledgeTable({
  ...
  const filtered = useMemo(() => {
    ...
    return articles.filter((article) => {
      if (onlyPublished && !article.published) return false;
  ...
  }, [articles, search, onlyPublished]);

  const handleDelete = async (id: string) => {
    if (!confirm('Diesen Wissenswert-Artikel wirklich löschen?')) {
      return;
    }
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <h2 className="text-lg font-semibold">
              Wissenswertes ({filtered.length} von {articles.length})
            </h2>
            {isPending && (
              <p className="text-sm text-gray-500 mt-1">Aktualisiere Liste…</p>
            )}
          </div>
